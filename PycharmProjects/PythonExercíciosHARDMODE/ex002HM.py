...
while op != 'N':
    ...
    print('')
    op = (str(input('Deseja digitar um novo nome? A mensagem de boas vindas pode ser diferente! [S/N]'))).strip().upper()[0]
    if op != 'S' and op != 'N':
        while op != 'S' and op != 'N':
            op = (str(input('OPÇÃO INVÁLIDA! Deseja digitar um novo nome? A mensagem de boas vindas pode ser diferente! [S/N]'))).strip().upper()[0]
